...
def make_xlsx ():
    ...
    for i in range(0,len(all_LMZ)):
        ...
        if len(file_list) > 0:
            a.append('=HYPERLINK("C:/Users/junsu/Desktop/Project/results/png_files/Analysis_{0}_({1},{2})_{3}_{4}.png","show '
                     'png")'.format(
                TestSiteInfo(all_LMZ[i], "Wafer"),
                TestSiteInfo(all_LMZ[i], "DieRow"),
                TestSiteInfo(all_LMZ[i], "DieColumn"),
...
def choice_iv():
    while True:
        a = input("what is wafer(D07,D08,D23,D24)? : " )
        if a == "D07":
            for i in range(0, len(all_LMZ)):
                if TestSiteInfo(all_LMZ[i],"Wafer") == "D07":
                    iv.iv(all_LMZ[i])
                    plt.savefig("./results/png_files/Analysis_{}_({},{})_{}_{}_IV.png"
                                .format(TestSiteInfo(all_LMZ[i], "Wafer"),
                                        TestSiteInfo(all_LMZ[i], "DieRow"),
            ...
            for i in range(0, len(all_LMZ)):
                if TestSiteInfo(all_LMZ[i],"Wafer") == "D23":
                    iv.iv(all_LMZ[i])
                    plt.savefig("./results/png_files/Analysis_{}_({},{})_{}_{}_IV.png"
                                .format(TestSiteInfo(all_LMZ[i], "Wafer"),
                                        TestSiteInfo(all_LMZ[i], "DieRow"),
            ...
            for i in range(0, len(all_LMZ)):
                if TestSiteInfo(all_LMZ[i],"Wafer") == "D24":
                    iv.iv(all_LMZ[i])
                    plt.savefig("./results/png_files/Analysis_{}_({},{})_{}_{}_IV.png"
                                .format(TestSiteInfo(all_LMZ[i], "Wafer"),
                                        TestSiteInfo(all_LMZ[i], "DieRow"),
